...
(* parse a Program from a string *)
  ...
  val prog_parse : string -> Absyn.tree

(* parse a query from a string *)

  val query_parse : string -> Absyn.tree

(* parse a Program in a file *)

  val file_parse : string -> Absyn.tree
 
(* parse a query from the standard input *)

  val top_parse : unit -> Absyn.tree
...
functor Parse (structure Absyn : ABSYN
		  ...
		  sharing type Parser.result = Absyn.tree
...
fun parse (dummyToken,lookahead,reader : int -> string) =
	...
	fun invoke lexer = 
	   ...
	   val _ = Compiler.Control.Print.printDepth:=99;
...
fun top_parse () = 
    parse (Tokens.PARSEQUERY,0,(fn i => getOpt(TextIO.inputLine TextIO.stdIn,"")))
